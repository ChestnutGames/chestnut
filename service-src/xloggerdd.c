...
#define DEFAULT_ROLL_SIZE (128*ONE_MB)		// 日志文件达到512M，滚动一个新文件
#define DEFAULT_PATH      ("logs")
#define DEFAULT_INTERVAL  (5)			    // 日志同步到磁盘间隔时间

struct xloggerdd {
	logger_level loglevel;
	size_t rollsize;        // 文件多大时候滚蛋
	FILE* handle[LOG_MAX];
	size_t written_bytes[LOG_MAX];	// 已写入文件的字节数
...
xloggerdd_flush(struct xloggerdd *self) {
	...
	list_for_each_safe(pos, n, &self->head) {
		...
		skynet_free(CAST_MESSAGE_POINTER(request));
	}
