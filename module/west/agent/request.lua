...
function REQUEST:match(args, ... )
	...
	return skynet.call(".MATCH", "lua", "enter", uid, secret, skynet.self(), args.mode)
end

----------------------room----------------------------------
local function forward_room(name, ctx, args, ... )
	-- body
	local addr = assert(ctx:get_room())
	if addr then
		local command = "on_"..name
		log.info("route request command %s agent to room", command)
		return skynet.rawcall(addr, "lua", command, args)
	end
end
