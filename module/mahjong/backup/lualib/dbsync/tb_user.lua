...
function _M.cache_insert(db, uid, ... )
	...
	local openid         = db:get(string.format("%s:%d:openid", tname, openid))
	local nameid         = db:get(string.format("%s:%d:nameid", tname, nameid))

	local sql = "insert into %s (uid, gold, diamond, checkin_month, checkin_count, checkin_mcount, checkin_lday, rcard, sex, nickname, province, city, country, headimg, openid, nameid) values (%d, %d, %d, %d, %d, %d, %d, %d, %d, '%s', '%s', '%s', '%s', '%s', '%s', '%s')"
	sql = string.format(sql, tname, uid, gold, diamond, checkin_month, checkin_count, checkin_mcount, checkin_lday, rcard, sex, nickname, province, city, country, headimg, openid, nameid)
	log.info(sql)
